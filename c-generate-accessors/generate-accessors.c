 ...
 *
 * gcc -O2 generate-accessors.c -o generate-accessors
 *
 * Example usage:
 *   ./generate-accessor example_structs.h
 *
...
static char *safe_strdup(const char *s) {
	...
	if (!new_string) return NULL; /* Return NULL on allocation failure */
...
static bool mkdir_p(const char *path, mode_t mode) {
	...
	len = strlen(tmp);
	if (tmp[len - 1] == '/') tmp[len - 1] = '\0';  /* remove trailing slash */
	...
	if (mkdir(tmp, mode) != 0) {
		if (errno != EEXIST) goto mkdir_out;
	}
...
typedef struct StringList {
	char   **strings;  /* Pointer to an array of char pointers */
...
static void strlst_init(StringList_t *list, size_t initial_capacity) {
	...
	list->strings = (char **)calloc(initial_capacity, sizeof(char *));
	list->count = 0;
	list->capacity = initial_capacity;
}

/**
 * @brief Append a copy of a string to a StringList_t.
 *
 * If list capacity is exhausted, it doubles the capacity and
 * reallocates. The input string is duplicated (safe_strdup) and stored.
 *
 * @param list Pointer to the string list.
 * @param new_string Null-terminated string to add.
 */
static void strlst_add(StringList_t *list, const char *new_string) {
	...
	list->strings[list->count++] = safe_strdup(new_string);
}
 ...
 * @param filename Path to the file to read. If NULL, the function
 *                 returns immediately.
 */
 ...
 *         false otherwise.
 */
static bool is_excluded(const StringList_t *excl_list, const char *struct_name, const char *member_name) {
	char key[LINE_MAX];
 ...
 */
static StructInfo_t *struct_list_struct_add(StructList_t *sl, char *struct_name, bool steal) {
	StructInfo_t *si;

	if (sl->count == sl->capacity) {
		/* Reallocate to increase capacity */
		sl->capacity *= 2; /* Double the capacity */
		sl->items = (StructInfo_t *)realloc(sl->items, sl->capacity * sizeof(StructInfo_t));
...
static void struct_list_parse(StructList_t *sl, const char *text, Conf_t *conf) {
	...
	for (cursor = text, rc = regexec(&conf->re.re_struct, cursor, 4, regmatch_struct, 0);
		...
		for (line = strtok(body, "\n"); line != NULL; line = strtok(NULL, "\n")) {
			...
			if (trimmed_line[0] == '\0') continue; /* empty line */
			if (!strchr(trimmed_line, ';')) continue; /* skip lines without semicolon */
			if (strstr(trimmed_line, "static")) continue; /* skip static members */
 ...
 * of @si to the provided output FILE (@generated_hdr). If @conf->args.combine
 * is false, a header guard and includes are written for a standalone
 * header targeting a single struct. If @conf->args.combine is true,
 * only a comment block is emitted for the struct.
 *
	...
	for (size_t m = 0; m < si->count; m++) {
		...
		if (!members->is_const) { /* No setter on const members */
			if (members->is_char_array || streq(members->type, "const char *"))
				fprintf(generated_hdr, "void %s%s_%s_set(struct %s *p, const char *%s);\n",
					conf->args.prefix, si->name, members->name, si->name, members->name);
			else
				fprintf(generated_hdr, "void %s%s_%s_set(struct %s *p, %s %s);\n",
					conf->args.prefix, si->name, members->name, si->name, members->type, members->name);
		}
...
static void generate_src(FILE  *generated_src, StructInfo_t  *si, Conf_t  *conf) {
	for (size_t m = 0; m < si->count; m++) {
		...
		/* Setter method */
		if (!member->is_const) {
			if (!member->is_char_array && streq(member->type, "const char *")) { /* dynamic string */
				fprintf(generated_src,
					"void %s%s_%s_set(struct %s *p, const char *%s) {\n"
					"    free(p->%s);\n"
					"    p->%s = %s ? strdup(%s) : NULL;\n"
					"}\n\n",
					conf->args.prefix, si->name, member->name, si->name, member->name,
					member->name,
					member->name, member->name, member->name);
			} else if (member->is_char_array) { /* fixed-size array */
				int dont_care;
				(void)dont_care;
				...
				fprintf(generated_src,
					...
					"    p->%s = %s;\n"
					"}\n\n",
					conf->args.prefix, si->name, member->name, si->name, member->type, member->name,
	...
	for (int i = optind; i < argc; ++i) {
		...
		if (ret == 0) {
			for (size_t j = 0; j < glob_result.gl_pathc; ++j) {
				strlst_add(&args->hdr_files, glob_result.gl_pathv[j]);
			}
		} else {
 ...
 * generates accessor header/source files either combined or per-struct
...
int main(int argc, char *argv[]) {
	...
	mkdir_p(conf.args.outdir, S_IRUSR|S_IWUSR|S_IXUSR|S_IRGRP|S_IXGRP|S_IROTH|S_IXOTH);

	if (conf.args.combine) {
		char *fname;
		int len;

		len = asprintf(&fname, "%s/accessors.h", conf.args.outdir);
		generated_hdr = fopen(fname, "w");

		fname[len - 1] = 'c'; /* Change the file name extension to .c */
		generated_src = fopen(fname, "w");
		free(fname);
	...
	STRLST_FOREACH(&conf.args.hdr_files, in_hdr) {
		...
		if (conf.args.combine) {
			fprintf(generated_hdr, "#include \"%s\"\n", in_hdr_fname);
			...
			STRUCT_LIST_FOREACH(&sl, si) {
				...
				len = asprintf(&fname, "%s/%s_accessors.h", conf.args.outdir, si->name);
				generated_hdr = fopen(fname, "w");
				...
				sname_upper = to_uppercase(safe_strdup(si->name));
				...
				printf("Generated %s_accessors.h/.c\n", si->name);
			}
		}
	}
